  ...
  <div>
    <div class="row">
      <div class="col-12 m-0 p-0">
        <div class="titulogo">
          <div class="titulo">Redes de Conocimiento</div>
          <svg
            class="logo"
      ...
      <div v-if="useRed.cargando == false" class="row">
        <div class="col-12">
          <div class="table-container" style="text-transform: capitalize">
            <table class="table table-striped">
              ...
              <tbody>
                <tr class="my-4 tds" v-for="r in redesEncontrados" :key="r">
                  <td
                    data-bs-toggle="modal"
...
function cambio(msg) {
  if (msg == "Agregar") {
    limpiar();
    msg_modal.value = "Agregar Red";
  } else {
    msg_modal.value = "Editar Red";
  }
}

async function buscarRedes() {
  await useRed.buscarRed().then((res) => {
    redesEncontrados.value = res;
    newRedes.value = [...redesEncontrados.value]
  });
}

function red_filtrado(event){
  ...
  if (buscado_v.value === "") {
    ...
    redesEncontrados.value = newRedes.value;
    let filtrado = redesEncontrados.value.filter((g) =>
      g.nombre.includes(buscado_v.value.toLowerCase())
    );
    if (filtrado.length === 0) {
      vacio.value = `Sin resultados para ${buscado_v.value}`;
      redesEncontrados.value = [];
    } else {
      vacio.value = "";
      redesEncontrados.value = filtrado;
    }
  }
}
...
function editar(r) {
  (id.value = r._id), (nombre.value = r.nombre);
}

async function guardarEdision() {
  if(validaciones()===true){
  await useRed
    .editarRed(id.value, nombre.value.toLowerCase())
    .then(() => {
      buscarRedes();
      ok();
      closeModal();
...
async function cambiarEstado(a) {
  ...
  await useRed.cambiarEstado(a._id, a.estado).then(() => {
...
onMounted(async () => {
  setDocumentTitle();
  await buscarRedes(), await traigodatos();
});
