<template>
  <div>
    <div class="row" style="margin-top: 70px">
      <div class="row">
        <div class="col-12 contenedor">
          ...
          <span class="titulo">Investigación</span>
          <svg
            class="logo"
      ...
      <div class="row" style="margin-top: 30px">
        <div class="col-12 buscar">
          <input
            v-model="buscado_v"
            type="text"
            @input="investigacion_filtrado"
            placeholder="Buscar por nombre"
          />
          <img src="../assets/buscar.png" alt="" />
          <div v-if="vacio !== ''">{{ vacio }}</div>
    ...
    <div class="row" v-if="useInvestigacion.cargando === false">
      <div class="col-12">
          ...
          <table class="table table-striped">
            ...
            >
              <tr class="my-4">
                ...
                <td>{{ i.descripcion }}</td>
                <td>{{ i.fecha }}</td>
                <td @click="cambiarEstado(i)" style="cursor: pointer">
                  <span v-if="i.estado === true" style="color: green"
                    >Activo</span
                  >
                  <span style="color: red" v-else>Inactivo</span>
                </td>
                <td>
                  <a :href="i.documento" target="_blank"
                    ><img
        ...
        <div class="modal-content">
          ...
          <div class="modal-body d-flex flex-column">
            ...
            <label for="mi-archivo">
              ...
              <span style="margin: 0 10px" v-if="documento === null"
                >Archivo</span
              >
              <span style="margin: 0 10px" v-else>{{ documento.name }}</span>
            </label>
          </div>
          ...
          <div
            v-if="useInvestigacion.cargando === true"
          ...
          <div class="modal-footer" style="margin: auto">
            ...
            <button
              ...
              v-if="msg_modal == 'Agregar Investigación'"
              type="button"
              class="btn"
              @click="registrarInvestigacion()"
            >
              Agregar
            </button>
            <button
...
import { useInvestigacionStore } from "../stores/investigacion.js";
...
function cambio(msg) {
  if (msg === "Agregar") {
    msg_modal.value = "Agregar Investigación";
  } else {
    msg_modal.value = "Editar Investigación";
  }
}

async function buscarInvestigaciones() {
  try {
    let buscar = await useInvestigacion.buscarInvestigaciones();
    investigaciones.value = buscar;
...
const investigacion_filtrado = computed(() => {
  if (buscado_v.value === "") {
    vacio.value = "";
    buscarInvestigaciones();
  } else {
    let filtrado = investigaciones.value.filter((g) =>
      g.nombre.includes(buscado_v.value)
    );
    ...
    if (filtrado.length === 0) {
      ...
      vacio.value = "";
      investigaciones.value = filtrado;
    }
  }
});
...
async function registrarInvestigacion() {
  await useInvestigacion
...
function editar(i) {
  ...
  (fecha.value = i.fecha),
    (documento.value = i.documento),
    (programa.value = i.programa._id);
}
...
async function cambiarEstado(i) {
  if (i.estado === true) {
    i.estado = false;
  } else {
    i.estado = true;
  ...
  try {
    await useInvestigacion.cambiarEstado(i._id, i.estado);
  } catch (error) {
    console.log(error);
  }
}
...
onMounted(async () => {
  await buscarInvestigaciones(), traigodatos(), setDocumentTitle();
});
