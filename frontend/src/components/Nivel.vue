  ...
  <div id="app">
    <div class="row">
      <div class="row">
        <div class="col-12">
          <div class="titulogo">
            <h1 class="titulo">Niveles de formación</h1>
    ...
    <div v-if="useNivel.cargando == false">
      <div class="row">
        <div class="col-12">
          <div class="table-container" style="text-transform: capitalize">
            <table class="table table-striped">
              ...
              <tbody>
                <tr v-for="n in nivelesEncontrados" :key="n">
                  <td
                    data-bs-toggle="modal"
                  ...
                  <td>{{ n.descripcion }}</td>
                  <td @click="cambiarEstado(n)">
                    <strong v-if="n.estado === true" style="color: green"
                      >Activo</strong
                    >
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-footer" style="margin: auto">
            ...
            <button
              v-if="msg_modal == 'Agregar Nivel'"
              type="button"
              class="btn"
              @click="registrarNivel()"
              :style="{
...
function cambio(msg) {
  if (msg == "Agregar") {
    limpiar();
    msg_modal.value = "Agregar Nivel";
  } else {
    msg_modal.value = "Editar Nivel";
  }
}

async function buscarNivel() {
  await useNivel.buscarNivel().then((res) => {
    nivelesEncontrados.value = res;
    newNivel.value = [...nivelesEncontrados.value];
  });
}

function nivel_filtrado(event) {
  buscado_v.value = event.target.value;
  if (buscado_v.value === "") {
    buscarNivel();
    vacio.value = "";
  } else {
    nivelesEncontrados.value = newNivel.value;
    let filtrado = nivelesEncontrados.value.filter((g) =>
      g.denominacion.includes(buscado_v.value.toLowerCase())
    );
    nivelesEncontrados.value = [];
    if (filtrado.length === 0) {
      vacio.value = `Sin resultados para ${buscado_v.value}`;
    } else {
      vacio.value = "";
      nivelesEncontrados.value = filtrado;
    }
  }
}
...
async function registrarNivel() {
  if (validaciones() === true) {
    await useNivel
      .registrarNivel({
        denominacion: nombre.value.toLowerCase(),
...
function editar(n) {
  id.value = n._id;
  (nombre.value = n.denominacion), (descripcion.value = n.descripcion);
}
...
async function cambiarEstado(n) {
  ...
  await useNivel.cambiarEstado(n._id, n.estado).then(() => {
...
onMounted(async () => {
  setDocumentTitle(), await buscarNivel(), await traigodatos();
});
