<template>
  <div>
    <div class="row" style="margin-top: 70px">
      <div class="row" style="background-color: rgb(243, 243, 247)">
          ...
          <div>
            <div class="table-container">
              <table class="table table-striped">
                ...
                <tbody v-for="u in materialesFormacion" :key="u">
                  <tr>
                    <td>{{ u.nombre }}</td>
                    <td>{{ u.tipo }}</td>
                    <td>{{ u.descripcion }}</td>
                    <td>
                      <a :href="u.archivo" target="_blank"
                        ><img
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-body" style="margin: 20px">
            <span style="margin: 10px">Selecione el Material de Formación</span>
            <select v-model="materialId">
              <option value="" disabled selected>Seleccione el Material de Formacion</option>
              <option :value="m._id" v-for="m in materialEncontrado" :key="m">
                {{ m.codigo }} {{ m.nombre }}
              </option>
            </select>
          </div>
          ...
          <div class="modal-footer" style="margin: auto">
            ...
            <button
              ...
              @click="agregarMaterialFormacionPrograma()"
            >
              Agregar
            </button>
          </div>
...
async function materialesPrograma() {
  ...
  materialesFormacion.value = usePrograma.idPrograma.materialformacion;
  newMatriales.value = [...materialesFormacion.value];
}
...
function material_filtrado(event) {
  ...
  if (buscado_v.value.trim() === "") {
    ...
    materialEncontrado.value = newMatriales.value;
...
async function agregarMaterialFormacionPrograma() {
  if(materialId.value.trim()===''){
    alert.value = true;
        errores.value = 'Debe seleccionar un Material de formación';
        alerta();
  }else{
    await usePrograma
    .agregarMaterialFormacion(usePrograma.idPrograma._id, materialId.value)
    .then(() => {
      materialesPrograma();
      ok();
...
onMounted(async () => {
  await materialesPrograma(), traigodatos(), setDocumentTitle();
});
