<template>
  <div>
    <div class="row">
      <div class="titulogo">
        <router-link style="margin-top: 15px;" to="guia">
          <img
        ...
        <p class="titulo">Instrumentos de Evaluación</p>
        <svg
          class="logo"
      ...
      <div class="row" style="margin-top: 2%; margin-bottom: 2%;">
        ...
        <div v-if="useUsuarios.rol == 'Gestor'.toLowerCase()">
          <button
            type="button"
      ...
      <div class="row" v-if="useGuia.cargando === false">
        <div class="col-12" v-if="useGuia.idGuia.evaluacion">
          <div class="table-container">
            <table class="table">
              ...
              <tbody
                ...
                v-for="e in evaluacionesEncontradas"
                :key="e"
              >
                <tr class="my-4">
                  <td
                  ...
                  >
                    <span v-if="e.estado === true" style="color: green"
                      >Activo</span
                    >
                  ...
                  <td v-if="useUsuarios.rol === 'Gestor'.toLowerCase()">
                    <img
                      ...
                      data-bs-toggle="modal"
                      data-bs-target="#staticBackdrop"
                      @click="cambio('Editar'), editar(e)"
                      src="../assets/editar1.png"
                      alt=""
                    />
                  </td>
                  <td>
                    <a :href="e.archivo" target="_blank">
                      <img
                        style="width: 30px; margin: 10px; cursor: pointer"
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-body d-flex flex-column">
            ...
            <label :style="{backgroundColor: coloroficialmenu, color: coloroficialtres}" for="mi-archivo">
              <svg
                xmlns="http://www.w3.org/2000/svg"
          ...
          <div
            v-if="useEvaluacion.cargando === true"
            class="row d-flex justify-content-center"
          >
            <div class="spinner-border text-success" role="status">
              <span class="visually-hidden">Loading...</span>
...
let evaluacionesEncontradas = ref([]);
let buscado_v = ref("");
let vacio = ref("");
let msg_modal = ref("");
let nombre = ref("");
...
function cambio(msg) {
  if (msg == "Agregar") {
    msg_modal.value = "Agregar Evaluación";
    limpiar();
  } else {
    msg_modal.value = "Editar Evaluación";
  }
}

async function buscarEvaluacionId() {
  await useGuia.buscarGuiaId(useGuia.idGuia._id);
  evaluacionesEncontradas.value = useGuia.idGuia.evaluacion;
  newEvaluacion.value = [...evaluacionesEncontradas.value]
}

function evaluacion_filtrado(event){
  buscado_v.value=event.target.value;
  if (buscado_v.value.trim() === "") {
    buscarEvaluacionId();
    vacio.value = "";
  } else {
    evaluacionesEncontradas.value = newEvaluacion.value;
    let filtrado = evaluacionesEncontradas.value.filter((g) =>
      g.nombre.includes(buscado_v.value.trim().toLowerCase())
    );
    if (filtrado.length === 0) {
      vacio.value = `Sin resultados para ${buscado_v.value}`;
      evaluacionesEncontradas.value = "";
    } else {
      vacio.value = "";
      evaluacionesEncontradas.value = filtrado;
    }
  }
}
...
async function registrarEvaluacion() {
  if(validaciones()===true){
    await useGuia
    .agregarEvaluacion(useGuia.idGuia._id, {
...
function editar(e) {
  id.value = e._id;
  (nombre.value = e.nombre), (archivo.value = e.archivo);
}

async function guardarEdision() {
  if(validaciones()===true){
    await useEvaluacion
    .editarEvaluacion(id.value, nombre.value.toLowerCase(), archivo.value)
    .then(() => {
      buscarEvaluacionId();
      ok();
      closeModal();
