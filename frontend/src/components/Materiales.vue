  ...
  <div>
    <div class="row">
      <div class="contenedor">
        <div class="col-12">
          <router-link style="padding-left: 10px; margin: 0 50px" to="guia">
            <img
          ...
          <span class="titulo">Material de Apoyo</span>
          <svg
            class="logo"
      ...
      <div class="row p-3">
        <div class="col-9 buscar">
          <input
            class="input"
            @input="material_filtrado"
            type="text"
            placeholder="Buscar Material por codigo"
          />
          <img src="../assets/buscar.png" alt="" />
          <div class="alert-vacio" v-if="vacio !== ''">{{ vacio }}</div>
        </div>

        <div class="col-3" v-if="useUsuarios.rol === 'Gestor'.toLowerCase()">
          <button
            type="button"
            class="btn btn-success botonagregar"
      ...
      <div v-if="useGuia.cargando == false">
        <div class="row" v-if="useGuia.idGuia.material">
          <div class="accordion accordion-flush" id="accordionFlushExample">
            <div
              class="accordion-item"
              v-for="m in materialesEncontrados"
              :key="m"
            >
              <h2 class="accordion-header">
                <button
                  ...
                  :data-bs-target="'#collapse-' + m._id"
                  aria-expanded="false"
                  :aria-controls="'collapse-' + m._id"
              ...
              >
                <div class="accordion-body">
                  <p>
                    <span style="font-weight: 1000">Nombre:</span>
                    {{ m.nombre }}
                  </p>
                  <div style="display: flex">
                    <p>
                      <span style="font-weight: 1000">Estado:</span>
                      <strong v-if="m.estado === true" style="color: green">
                        Activo</strong
                      >
                    ...
                    <button
                      ...
                      style="margin-left: 30%; margin-right: 2%"
                    ...
                    <button
                      ...
                      class="btn btn-secondary"
                    >
                      <a :href="m.archivo" target="_blank">
                        <img
    ...
    >
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div
            v-if="useMaterial.cargando === true"
            class="row d-flex justify-content-center"
          >
            <div class="spinner-border text-success" role="status">
              <span class="visually-hidden">Loading...</span>
          ...
          <div class="modal-footer" style="margin: auto">
            ...
            <button
              v-if="msg_modal == 'Agregar Material de apoyo'"
              type="button"
...
const useGuia = useGuiaStore();
const useUsuarios = useUsuarioStore();

let materialesEncontrados = ref([]);
let buscado_v = ref("");
let vacio = ref("");
let msg_modal = ref("");
let id = ref("");
...
function cambio(msg) {
  if (msg == "Agregar") {
    limpiar();
    msg_modal.value = "Agregar Material de apoyo";
  } else {
    msg_modal.value = "Editar Material de apoyo";
  }
}

async function buscarMaterialId() {
  await useGuia.buscarGuiaId(useGuia.idGuia._id);
  materialesEncontrados.value = useGuia.idGuia.material;
  newMateriales.value = [...materialesEncontrados.value];
}

function material_filtrado(event) {
  buscado_v.value = event.target.value;
  if (buscado_v.value.trim() === "") {
    buscarMaterialId();
    vacio.value = "";
  } else {
    materialesEncontrados.value = newMateriales.value;
    let filtrado = materialesEncontrados.value.filter((g) =>
    ...
    if (filtrado.length === 0) {
      ...
      vacio.value = "";
      materialesEncontrados.value = filtrado;
    }
  }
}
...
async function registrarMaterial() {
  if (validaciones() === true) {
    await useGuia
      .agregarMaterial(useGuia.idGuia._id, {
...
function editar(m) {
  (id.value = m._id), (nombre.value = m.nombre), (archivo.value = m.archivo);
}

async function guardarEdision() {
  if (validaciones() === true) {
    await useMaterial
      .editarMaterial(id.value, nombre.value.toLowerCase(), archivo.value)
      .then(() => {
        buscarMaterialId();
        ok();
        closeModal();
...
async function cambiarEstado(m) {
  ...
  await useMaterial.cambiarEstado(m._id, m.estado).then(() => {
...
.accordion-button.collapsed:focus {
  background-color: #b0edc8 !important;
