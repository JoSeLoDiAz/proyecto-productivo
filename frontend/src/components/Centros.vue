<template>
  <div>
    <div class="row" style="margin-top: 70px">
      <div class="row">
        <div class="col-12 contenedor">
          <h1 :style="{ color: coloroficialletra }" class="titulo">Centros</h1>
          <svg
            class="logo"
      ...
      <div class="row" style="margin-top: 30px">
        <div class="col-12 buscar">
          <input
            @input="centro_filtrado"
            type="text"
            placeholder="Buscar Centros por codigo"
          />
    ...
    <div class="row" v-if="useCentro.cargando === false">
      <div class="col-12">
        <div class="table-container">
          <table
            v-if="centrosEncontrados.length > 0"
          ...
          >
            ...
            <tbody
              class="align-items-center capiTabla"
              v-for="c in centrosEncontrados"
              :key="c"
            >
              <tr class="my-4">
                <td>{{ c.codigo }}</td>
                <td data-bs-toggle="modal" data-bs-target="#perfil">
                  {{ c.nombre }}
                </td>
                <td>{{ c.direccion }}</td>

                <td>
                  <router-link to="/sedes">
                    <img
                      @click="irsedes(c)"
                      v-if="c.nombre"
                ...
                <td style="cursor: pointer" @click="cambiarEstado(c)">
                  <strong v-if="c.estado === true" style="color: green"
                    >Activo</strong
                  >
                  <strong v-else style="color: red">Inactivo</strong>
                </td>
                <td>
                  <img
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div v-if="alert === true">
            ...
            <div class="alert alert-warning d-flex capiTabla" role="alert">
              <svg
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-body d-flex flex-column">
            ...
            <span>Selecione la ciudad</span>
            <select v-model="ciudadId">
              <option value="" disabled selected v-if="msg_modal === 'Agregar Centro'">
                Seleccione
              </option>
              <!-- <option value="" v-else>{{ ciudades_filtradas.nombre }}</option> -->
              <option :value="c._id" v-for="c in ciudades_filtradas" :key="c">
                {{ c.nombre }}
              </option>
            </select>
          </div>
          ...
          <div class="modal-footer" style="margin: auto">
            ...
            <button
              ...
              v-if="msg_modal == 'Agregar Centro'"
              type="button"
              class="btn"
              @click="registrarCentro()"
            >
              Agregar
            </button>
            <button
...
async function buscocentro() {
  try {
    const res = await useCentro.buscarCentro();
    centrosEncontrados.value = res;
    newCentros.value=[...centrosEncontrados.value];
  } catch (error) {
    console.error("Error al buscar el centro:", error);
  }
}

async function irsedes(c) {
  await useCentro
    .buscarCentroId(c._id)
    .then(() => {
      console.log(useCentro.idCentro);
    })
    .catch((err) => {
      console.log(err);
    });
...
function cambio(msg) {
  if (msg == "Agregar") {
    buscarCiudad();
    msg_modal.value = "Agregar Centro";
    limpiar();
  } else {
    msg_modal.value = "Editar Centro";
  }
}

async function buscarCiudad() {
  await useCiudad.buscarCiudad().then((res) => {
    ciudades.value = res;
    ciudades_filtradas.value = ciudades.value.filter(
      (ciudad) => ciudad.estado == true
    );
  });
};

function centro_filtrado(event){
buscado_v.value = event.target.value;
if (buscado_v.value.trim() === "") {
    buscocentro();
    vacio.value = "";
  } else {
    centrosEncontrados.value = newCentros.value
    let filtrado = centrosEncontrados.value.filter((g) =>
      g.codigo.includes(buscado_v.value.trim().toLowerCase())
    );
    if (filtrado.length === 0) {
      vacio.value = `Sin resultados para ${buscado_v.value}`;
      centrosEncontrados.value = [];
    } else {
      vacio.value = "";
      centrosEncontrados.value = filtrado;
    }
  }
};
...
function validaciones() {
  if(codigo.value.trim()===''){
    ...
    alerta();
  }else if(ciudadId.value.trim()===''){
    alert.value = true;
    errores.value = "Complete el campo Ciudad";
    alerta();
  }else{
    return true;
  }
...
async function editar(c) {
  ...
  (nombre.value = c.nombre),
    ...
    (ciudadId.value = c.ciudad._id);
  sedesId.value = c.sedes._id;
}
...
async function cambiarEstado(c) {
  if (c.estado === true) {
    c.estado = false;
  } else {
    c.estado = true;
  }
  await useCentro.cambiarEstado(c._id, c.estado).then(() => {
...
function limpiar() {
  codigo.value = "";
  (nombre.value = ""), (direccion.value = ""), (ciudadId.value = "");
}
...
onMounted(async () => {
  setDocumentTitle(), await buscocentro(), await traigodatos();
});
