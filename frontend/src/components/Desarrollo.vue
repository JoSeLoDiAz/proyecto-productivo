<template>
  <!------------------------------------- desarrollo----------------------------------- -->
  <div>
    <div class="titulogo" style="margin-bottom: 10px">
      <router-link to="infoprograma">
        <img
          src="../assets/volver.png"
          style="width: 45px; height: 45px; left: -200px; position: relative"
          alt=""
        />
      </router-link>
      <div class="titulo">Desarrollo curricular</div>
      <svg
        class="logo"
        xmlns:dc="http://purl.org/dc/elements/1.1/"
        xmlns:cc="http://creativecommons.org/ns#"
        xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"
        xmlns:svg="http://www.w3.org/2000/svg"
        xmlns="http://www.w3.org/2000/svg"
        xmlns:sodipodi="http://sodipodi.sourceforge.net/DTD/sodipodi-0.dtd"
        xmlns:inkscape="http://www.inkscape.org/namespaces/inkscape"
        width="100"
        height="98"
        viewBox="0 0 74.999997 73.5"
        version="1.1"
        id="svg12"
        sodipodi:docname="SENA logo.svg"
        inkscape:version="0.92.4 (5da689c313, 2019-01-14)"
      >
        <title id="title222">SENA Colombia logo</title>
        <metadata id="metadata18">
          <rdf:RDF>
            <cc:Work rdf:about="">
              <dc:format>image/svg+xml</dc:format>
              <dc:type rdf:resource="http://purl.org/dc/dcmitype/StillImage" />
              <dc:title>SENA Colombia logo</dc:title>
            </cc:Work>
          </rdf:RDF>
        </metadata>
        <defs id="defs16">
          <pattern
            y="0"
            x="0"
            height="6"
            width="6"
            patternUnits="userSpaceOnUse"
            id="EMFhbasepattern"
          />
          <pattern
            y="0"
            x="0"
            height="6"
            width="6"
            patternUnits="userSpaceOnUse"
            id="EMFhbasepattern-0"
          />
        </defs>
        <sodipodi:namedview
          pagecolor="#ffffff"
          bordercolor="#666666"
          borderopacity="1"
          objecttolerance="1"
          gridtolerance="1"
          guidetolerance="1"
          inkscape:pageopacity="0"
          inkscape:pageshadow="2"
          inkscape:window-width="1366"
          inkscape:window-height="705"
          id="namedview14"
          showgrid="false"
          units="px"
          inkscape:showpageshadow="false"
          inkscape:zoom="1"
          inkscape:cx="96.265104"
          inkscape:cy="56.747294"
          inkscape:window-x="-8"
          inkscape:window-y="-8"
          inkscape:window-maximized="1"
          inkscape:current-layer="svg12"
        >
          <inkscape:grid type="xygrid" id="grid58" />
        </sodipodi:namedview>
        <g id="#ffffffff" transform="translate(0,-130.5)" />
        <path
          :style="{ fill: coloroficialcuatro }"
          d="M 37.729312,0.76709211 A 7.9671225,7.9671225 0 0 0 29.773371,8.741545 7.9671225,7.9671225 0 0 0 37.744605,16.701107 7.9671225,7.9671225 0 0 0 45.707788,8.733898 V 8.718608 A 7.9671225,7.9671225 0 0 0 37.729312,0.76709211 Z M 11.446982,19.169553 c -1.400062,0.0084 -2.8234862,0.08599 -4.1584857,0.42261 -0.8795905,0.222694 -1.7315449,0.589234 -2.2841086,1.159562 -0.6939573,0.71469 -0.7830516,1.683375 -0.4447463,2.510708 0.3027378,0.728932 1.116412,1.275934 2.0228936,1.588612 1.9500175,0.668079 4.117455,0.809612 6.168096,1.230399 0.378207,0.09257 0.796556,0.194498 1.030766,0.452796 0.242885,0.306852 0.09871,0.729986 -0.299446,0.922901 -0.666204,0.338572 -1.517067,0.341289 -2.29256,0.329636 -0.708709,-0.02913 -1.4860247,-0.08878 -2.0498634,-0.44354 -0.4146382,-0.255061 -0.487253,-0.689278 -0.3944343,-1.062161 l -4.5714374,4.13e-4 c -0.012144,0.695917 0.1230528,1.422157 0.6331108,2.019674 0.4259168,0.510124 1.1138561,0.869443 1.8546552,1.08631 1.1823274,0.347635 2.4633397,0.45247 3.7246061,0.484191 1.711468,0.02913 3.456086,-0.02172 5.099895,-0.410133 0.979349,-0.238231 1.942311,-0.624278 2.569473,-1.250927 1.113798,-1.117998 0.851643,-2.888599 -0.629086,-3.758011 -0.737329,-0.429203 -1.620362,-0.695805 -2.52077,-0.867357 -1.316781,-0.273188 -2.657724,-0.472977 -3.991855,-0.693081 -0.468421,-0.09127 -0.9637641,-0.17183 -1.3575867,-0.393229 -0.4120347,-0.221399 -0.4451988,-0.742521 -0.025355,-0.970393 0.5430207,-0.306203 1.2665147,-0.304035 1.9162377,-0.304681 0.68528,0.01165 1.430476,0.05483 1.994315,0.375921 0.31575,0.17673 0.442485,0.473443 0.445955,0.758283 l 4.344837,-0.01004 c -0.01734,-0.550925 -0.118731,-1.123123 -0.521227,-1.598289 -0.466686,-0.585218 -1.287477,-0.954052 -2.13237,-1.169625 -1.325457,-0.335334 -2.741002,-0.404061 -4.131515,-0.410535 z m 9.425817,0.328831 -0.0019,10.38132 12.670256,4.12e-4 -0.0011,-2.263178 H 25.518157 V 25.60169 h 7.154592 v -2.212465 h -7.154592 l -0.0016,-1.652607 7.734167,-0.002 -0.0044,-2.236213 z m 20.874932,0.0036 c 0,0 -3.91491,0.0012 -5.871868,0.0012 l -8.25e-4,10.382125 4.448274,4.12e-4 -0.0028,-6.986351 6.093235,6.980716 6.104905,0.0056 -0.0016,-10.382527 -4.451897,7.97e-4 0.0052,6.936443 z m 18.702713,0.01731 c 0,0 -4.795135,6.925757 -7.204899,10.383332 l 4.669239,4.12e-4 1.120118,-1.873169 h 7.21456 l 1.045256,1.873974 5.186434,-7.96e-4 -6.872848,-10.382125 z m 2.333214,2.478105 2.214074,3.76083 -4.576669,0.0065 z m -62.26819489,10.81881 0.0402459,5.651707 21.11883799,-0.07527 c 1.07706,0.231841 1.7024,0.932964 1.482355,2.526807 l -12.989427,22.73807 4.231335,3.955631 20.118662,-34.796918 z m 40.29930289,0.0483 19.784196,34.649208 4.372205,-3.930275 -13.14036,-22.678472 c -0.22011,-1.598763 0.405444,-2.307963 1.482759,-2.540491 l 21.123264,0.07527 -0.0056,-5.557928 z m -3.337814,5.733814 -18.552992,31.845491 4.928038,2.401635 12.380065,-20.924035 c 0.426522,-0.348923 0.858497,-0.534847 1.290772,-0.552212 0.455325,-0.01744 0.916655,0.151004 1.377709,0.511558 l 12.351487,20.988033 5.083398,-2.65158 z"
          id="path47-5"
          inkscape:connector-curvature="0"
        />
        <g
          id="#000000ff-2"
          transform="matrix(0.31570611,0,0,0.23560774,-391.49698,-10.601126)"
          style="fill: #ff6719; fill-opacity: 1"
        />
      </svg>
    </div>

    <!-- modal matriz de correlacion -->
    <div
      class="modal fade"
      id="matrizdecorrelacion"
      data-bs-backdrop="static"
      data-bs-keyboard="false"
      tabindex="-1"
      aria-labelledby="staticBackdropLabel"
      aria-hidden="true"
    >
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <div
              style="
                display: flex;
                justify-content: center;
                align-items: center;
                text-align: center;
              "
            >
              <p style="font-size: 25px; font-weight: 1000">
                Agregar matriz de correlacion
              </p>
            </div>
            <button
              type="button"
              class="btn-close"
              data-bs-dismiss="modal"
              aria-label="Close"
            ></button>
          </div>
          <div class="modal-body">
            <div class="input-group mb-3">
              <input
                type="file"
                accept=".pdf, .doc, .docx"
                class="form-control"
                id="inputGroupFile02"
                @change="llevarMatriz"
              />
            </div>
          </div>

          <div v-if="alert === true">
            <svg xmlns="http://www.w3.org/2000/svg" class="d-none alerta">
              <symbol id="exclamation-triangle-fill" viewBox="0 0 16 16">
                <path
                  d="M8.982 1.566a1.13 1.13 0 0 0-1.96 0L.165 13.233c-.457.778.091 1.767.98 1.767h13.713c.889 0 1.438-.99.98-1.767L8.982 1.566zM8 5c.535 0 .954.462.9.995l-.35 3.507a.552.552 0 0 1-1.1 0L7.1 5.995A.905.905 0 0 1 8 5zm.002 6a1 1 0 1 1 0 2 1 1 0 0 1 0-2z"
                />
              </symbol>
            </svg>
            <div
              class="alert alert-warning d-flex align-items-center"
              role="alert"
            >
              <svg
                style="width: 20px; height: 20px"
                class="bi flex-shrink-0 me-2"
                role="img"
                aria-label="Warning:"
              >
                <use xlink:href="#exclamation-triangle-fill" />
              </svg>
              <div>
                {{ errores }}
              </div>
            </div>
          </div>

          <div
            v-if="useDesarrollo.cargando === true"
            class="row d-flex justify-content-center"
          >
            <div class="spinner-border text-success" role="status">
              <span class="visually-hidden">Loading...</span>
            </div>
          </div>

          <div class="modal-footer" style="margin: auto">
            <button
              type="button"
              class="btn btn-danger"
              data-bs-dismiss="modal"
              id="close"
            >
              Cerrar
            </button>
            <button
              type="button"
              class="btn btn-success"
              @click="agregarMatriz()"
            >
              Guardar
            </button>
          </div>
        </div>
      </div>
    </div>

    <!-- Proyecto formativo -->

    <div
      class="modal fade"
      id="proyectoformativo"
      data-bs-backdrop="static"
      data-bs-keyboard="false"
      tabindex="-1"
      aria-labelledby="staticBackdropLabel"
      aria-hidden="true"
    >
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <p style="font-size: 25px; font-weight: 1000">Proyecto formativo</p>
            <button
              type="button"
              class="btn-close"
              data-bs-dismiss="modal"
              aria-label="Close"
            ></button>
          </div>
          <div class="modal-body">
            <div class="input-group mb-3">
              <input
                type="file"
                accept=".pdf, .doc, .docx"
                class="form-control"
                id="inputGroupFile02"
                @change="llevarProyecto"
              />
            </div>
          </div>

          <div v-if="alert === true">
            <svg xmlns="http://www.w3.org/2000/svg" class="d-none alerta">
              <symbol id="exclamation-triangle-fill" viewBox="0 0 16 16">
                <path
                  d="M8.982 1.566a1.13 1.13 0 0 0-1.96 0L.165 13.233c-.457.778.091 1.767.98 1.767h13.713c.889 0 1.438-.99.98-1.767L8.982 1.566zM8 5c.535 0 .954.462.9.995l-.35 3.507a.552.552 0 0 1-1.1 0L7.1 5.995A.905.905 0 0 1 8 5zm.002 6a1 1 0 1 1 0 2 1 1 0 0 1 0-2z"
                />
              </symbol>
            </svg>
            <div
              class="alert alert-warning d-flex align-items-center"
              role="alert"
            >
              <svg
                style="width: 20px; height: 20px"
                class="bi flex-shrink-0 me-2"
                role="img"
                aria-label="Warning:"
              >
                <use xlink:href="#exclamation-triangle-fill" />
              </svg>
              <div>
                {{ errores }}
              </div>
            </div>
          </div>

          <div
            v-if="useDesarrollo.cargando === true"
            class="row d-flex justify-content-center"
          >
            <div class="spinner-border text-success" role="status">
              <span class="visually-hidden">Loading...</span>
            </div>
          </div>

          <div class="modal-footer" style="margin: auto">
            <button
              type="button"
              class="btn btn-danger"
              data-bs-dismiss="modal"
              aria-label="Close"
              id="close2"
            >
              Cerrar
            </button>
            <button
              type="button"
              class="btn btn-success"
              @click="agregarProyecto()"
            >
              Guardar
            </button>
          </div>
        </div>
      </div>
    </div>

    <!-- Pedagogica -->

    <div
      class="modal fade"
      id="pedagogica"
      data-bs-backdrop="static"
      data-bs-keyboard="false"
      tabindex="-1"
      aria-labelledby="staticBackdropLabel"
      aria-hidden="true"
    >
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <p style="font-size: 25px; font-weight: 900">
              Planeacion pedagogica
            </p>
            <button
              type="button"
              class="btn-close"
              data-bs-dismiss="modal"
              aria-label="Close"
            ></button>
          </div>
          <div class="modal-body">
            <div class="input-group mb-3">
              <input
                type="file"
                accept=".pdf, .doc, .docx"
                class="form-control"
                id="inputGroupFile02"
                @change="llevarPlaneacion"
              />
            </div>
          </div>

          <div v-if="alert === true">
            <svg xmlns="http://www.w3.org/2000/svg" class="d-none alerta">
              <symbol id="exclamation-triangle-fill" viewBox="0 0 16 16">
                <path
                  d="M8.982 1.566a1.13 1.13 0 0 0-1.96 0L.165 13.233c-.457.778.091 1.767.98 1.767h13.713c.889 0 1.438-.99.98-1.767L8.982 1.566zM8 5c.535 0 .954.462.9.995l-.35 3.507a.552.552 0 0 1-1.1 0L7.1 5.995A.905.905 0 0 1 8 5zm.002 6a1 1 0 1 1 0 2 1 1 0 0 1 0-2z"
                />
              </symbol>
            </svg>
            <div
              class="alert alert-warning d-flex align-items-center"
              role="alert"
            >
              <svg
                style="width: 20px; height: 20px"
                class="bi flex-shrink-0 me-2"
                role="img"
                aria-label="Warning:"
              >
                <use xlink:href="#exclamation-triangle-fill" />
              </svg>
              <div>
                {{ errores }}
              </div>
            </div>
          </div>

          <div
            v-if="useDesarrollo.cargando === true"
            class="row d-flex justify-content-center"
          >
            <div class="spinner-border text-success" role="status">
              <span class="visually-hidden">Loading...</span>
            </div>
          </div>

          <div class="modal-footer" style="margin: auto">
            <button
              type="button"
              class="btn btn-danger"
              data-bs-dismiss="modal"
              aria-label="Close"
              id="close3"
            >
              Cerrar
            </button>
            <button
              type="button"
              class="btn btn-success"
              @click="agregarPlaneacion()"
            >
              Guardar
            </button>
          </div>
        </div>
      </div>
    </div>

    <div class="row">
      <div
        style="width: 90%"
        class="accordion accordion-flush"
        id="accordionFlushExample"
      >
        <div class="accordion-item">
          <h2>
            <button
              @click="urlArchivoMatriz()"
              class="accordion-button collapsed"
              type="button"
              data-bs-toggle="collapse"
              :data-bs-target="'#uno'"
              aria-expanded="false"
              :aria-controls="'uno'"
              style="font-weight: 900"
            >
              Matriz de correlacion
            </button>
          </h2>
          <div
            :id="'uno'"
            class="accordion-collapse collapse show"
            data-bs-parent="#accordionFlushExample"
          >
            <div class="accordion-body">
              <button
                v-if="useUsuario.rol === 'Gestor'.toLowerCase()"
                type="button"
                class="btn btn-secondary"
                data-bs-toggle="modal"
                data-bs-target="#matrizdecorrelacion"
              >
                <span class="adentroacordion" v-if="linkMatriz === true"
                  >Editar Matriz de Correlación
                </span>
                <span v-else>Agregar matriz de correlacion</span>
              </button>
              <a
                style="margin: 0 25px"
                v-if="linkMatriz === true"
                :href="urlMatriz"
                target="_blank"
              >
                <img
                  style="width: 30px"
                  src="../assets/descargar2.png"
                  alt=""
                />
              </a>
            </div>
          </div>
        </div>
      </div>
    </div>

    <div class="row">
      <div
        style="width: 90%"
        class="accordion accordion-flush"
        id="accordionFlushExample"
      >
        <div class="accordion-item">
          <h2>
            <button
              @click="urlArchivoProyecto()"
              class="accordion-button collapsed"
              type="button"
              data-bs-toggle="collapse"
              :data-bs-target="'#dos'"
              aria-expanded="false"
              :aria-controls="'dos'"
              style="font-weight: 1000"
            >
              proyecto formativo
            </button>
          </h2>
          <div
            :id="'dos'"
            class="accordion-collapse collapse"
            data-bs-parent="#accordionFlushExample"
          >
            <div class="accordion-body">
              <button
                v-if="useUsuario.rol === 'Gestor'.toLowerCase()"
                type="button"
                class="btn btn-secondary"
                data-bs-toggle="modal"
                data-bs-target="#proyectoformativo"
              >
                <span class="adentroacordion" v-if="linkProyecto === true"
                  >Editar Proyecto Formativo
                </span>
                <span v-else>Agregar proyecto formativo</span>
              </button>
              <a
                v-if="linkProyecto === true"
                style="margin: 0 25px"
                :href="urlProyecto"
                target="_blank"
              >
                <img
                  style="width: 30px"
                  src="../assets/descargar2.png"
                  alt=""
                />
              </a>
            </div>
          </div>
        </div>
      </div>
    </div>

    <div class="row">
      <div
        style="width: 90%"
        class="accordion accordion-flush"
        id="accordionFlushExample"
      >
        <div class="accordion-item">
          <h2>
            <button
              @click="urlArchivoPlaneacion()"
              class="accordion-button collapsed"
              type="button"
              data-bs-toggle="collapse"
              :data-bs-target="'#tres'"
              aria-expanded="false"
              :aria-controls="'tres'"
              style="font-weight: 1000"
            >
              Planeacion pedagogica
            </button>
          </h2>
          <div
            :id="'tres'"
            class="accordion-collapse collapse"
            data-bs-parent="#accordionFlushExample"
          >
            <div class="accordion-body">
              <button
                v-if="useUsuario.rol === 'Gestor'.toLowerCase()"
                type="button"
                class="btn btn-secondary"
                data-bs-toggle="modal"
                data-bs-target="#pedagogica"
              >
                <span class="adentroacordion" v-if="linkPlaneacion === true"
                  >Editar Planeación Pedagogica</span
                >
                <span v-else>Agregar planeación Pedagogica</span>
              </button>
              <a
                v-if="linkPlaneacion === true"
                style="margin: 0 25px"
                :href="urlPlaneacion"
                target="_blank"
              >
                <img
                  style="width: 30px"
                  src="../assets/descargar2.png"
                  alt=""
                />
              </a>
            </div>
          </div>
        </div>
      </div>
    </div>

    <div class="row">
      <div class="col-2"></div>
      <div
        style="
          display: flex;
          justify-content: center;
          align-items: center;
          text-align: center;
        "
      >
        <p class="col-10 segundotitulo">Guias de aprendizaje</p>
      </div>
    </div>

    <div class="contebotones">
      <button class="conteboton btn" type="button" @click="faseAnalisis()">
        Fase de Analisis
      </button>

      <button class="conteboton btn" type="button" @click="fasePlaneacion()">
        Fase de planeación
      </button>
      <button class="conteboton btn" type="button" @click="faseEjecucion()">
        Fase de Ejecucion
      </button>
      <button class="conteboton btn" type="button" @click="faseEvaluacion()">
        Fase de Evaluación
      </button>
    </div>
  </div>
</template>

<script setup>
import { ref, onMounted } from "vue";
import { useDesarrolloStore } from "../stores/desarrollo.js";
import { useProgramaStore } from "../stores/programa.js";
import { useUsuarioStore } from "../stores/usuario.js";
import { useRouter } from "vue-router";
import { useFaseStore } from "../stores/store.js";

const useDesarrollo = useDesarrolloStore();
const usePrograma = useProgramaStore();
const useUsuario = useUsuarioStore();
const router = useRouter();
const useFase = useFaseStore();

let id = ref("");
let matriz_correlacion = ref(null);
let proyecto = ref(null);
let planeacion = ref(null);
let alert = ref(false);
let errores = ref("");
let linkGuia = ref("");
let urlMatriz = ref("");
let urlProyecto = ref("");
let urlPlaneacion = ref("");
let linkMatriz = ref(false);
let linkProyecto = ref(false);
let linkPlaneacion = ref(false);

const setDocumentTitle = () => {
  document.title = `COLMABE || Desarrollo Curricular`;
};

function faseAnalisis() {
  useFase.setFase("analisis".toLowerCase());
  linkGuia.value = "/guia";
  router.push(linkGuia.value);
  if (useDesarrollo.faseAnalisis.length === 0) {
    console.log("Aun no hay guias en la fase de Analisis");
  }
}

function fasePlaneacion() {
  useFase.setFase("planeacion".toLowerCase());
  linkGuia.value = "/guia";
  router.push(linkGuia.value);
  if (useDesarrollo.fasePlaneacion.length === 0) {
    console.log("Aun no hay guias en la fase de Planeacion");
  }
}

function faseEjecucion() {
  useFase.setFase("ejecucion".toLowerCase());
  linkGuia.value = "/guia";
  router.push(linkGuia.value);
  if (useDesarrollo.faseEjecucion.length === 0) {
    console.log("Aun no hay guias en la fase de Ejecucion");
  }
}

function faseEvaluacion() {
  useFase.setFase("evaluacion".toLowerCase());
  linkGuia.value = "/guia";
  router.push(linkGuia.value);
  if (useDesarrollo.faseEvaluacion.length === 0) {
    console.log("Aun no hay guias en la fase de Evaluación");
  }
}
console.log(usePrograma.idPrograma);
const desarrolloId = async () => {
  try {
    await useDesarrollo.buscarDesarrolloId(
      usePrograma.idPrograma.desarrollo._id
    );
    id.value = useDesarrollo.idDesarrollo._id;
  } catch (error) {
    console.log(error);
  }
};

function llevarMatriz(event) {
  matriz_correlacion.value = event.target.files[0];
}

async function agregarMatriz() {
  await useDesarrollo
    .registrarMatriz(id.value, matriz_correlacion.value)
    .then(() => {
      desarrolloId();
      urlArchivoMatriz();
      useDesarrollo.buscarDesarrollo();
      ok();
      document.getElementById("close").click();
      limpiar();
    })
    .catch((err) => {
      if (err.response && err.response.data.errors) {
        alert.value = true;
        errores.value = err.response.data.errors[0].msg;
        alerta();
      } else if (err.response && err.response.data.msg) {
        alert.value = true;
        errores.value = err.response.data.msg;
        alerta();
      } else if (err.response.data.error) {
        alert.value = true;
        errores.value = "Falta el archivo";
        alerta();
      }
    });
}

async function urlArchivoMatriz() {
  if (
    urlMatriz.value !== "" ||
    urlMatriz.value !== undefined ||
    urlMatriz.value !== null
  ) {
    await useDesarrollo
      .archivoMatriz(useDesarrollo.idDesarrollo._id)
      .then((res) => {
        urlMatriz.value = res;
        linkMatriz.value = true;
      })
      .catch((err) => {
        console.log(err);
      });
  }
}

function llevarProyecto(event) {
  proyecto.value = event.target.files[0];
}

async function agregarProyecto() {
  await useDesarrollo
    .registrarProyecto(id.value, proyecto.value)
    .then(() => {
      desarrolloId();
      urlArchivoProyecto();
      useDesarrollo.buscarDesarrollo();
      ok();
      document.getElementById("close2").click();
      limpiar();
    })
    .catch((err) => {
      if (err.response.data.error) {
        alert.value = true;
        errores.value = "Falta el archivo";
        alerta();
      }
    });
}

async function urlArchivoProyecto() {
  if (
    urlProyecto.value !== "" ||
    urlProyecto.value !== undefined ||
    urlProyecto.value !== null
  ) {
    await useDesarrollo
      .archivoProyecto(useDesarrollo.idDesarrollo._id)
      .then((res) => {
        urlProyecto.value = res;
        linkProyecto.value = true;
      })
      .catch((err) => {
        console.log(err);
      });
  }
}

function llevarPlaneacion(event) {
  planeacion.value = event.target.files[0];
}

async function agregarPlaneacion() {
  await useDesarrollo
    .registrarplaneacion(id.value, planeacion.value)
    .then(() => {
      desarrolloId();
      urlArchivoPlaneacion();
      useDesarrollo.buscarDesarrollo();
      ok();
      document.getElementById("close3").click();
      limpiar();
    })
    .catch((err) => {
      if (err.response.data.error) {
        alert.value = true;
        errores.value = "Falta el archivo";
        alerta();
      }
    });
}

async function urlArchivoPlaneacion() {
  if (
    urlPlaneacion.value !== "" ||
    urlPlaneacion.value !== undefined ||
    urlPlaneacion.value !== null
  ) {
    await useDesarrollo
      .archivoPlaneacion(useDesarrollo.idDesarrollo._id)
      .then((res) => {
        urlPlaneacion.value = res;
        linkPlaneacion.value = true;
      })
      .catch((err) => {
        console.log(err);
      });
  }
}

function ok() {
  Swal.fire({
    position: "center",
    icon: "success",
    title: "Cambios guardados",
    showConfirmButton: false,
    timer: 1500,
  });
}

function limpiar() {
  matriz_correlacion.value = null;
  proyecto.value = null;
  planeacion.value = null;
}

function alerta() {
  setTimeout(() => {
    alert.value = false;
    errores.value = "";
  }, 1600);
}

onMounted(async () => {
  setDocumentTitle(), await desarrolloId(), traigodatos();
});

import { useConfiguracionStore } from "../stores/configuracion.js";
const useConfiguracion = useConfiguracionStore();
let coloroficialmenu = ref();
let coloroficialletra = ref();
let coloroficialtres = ref();
let coloroficialcuatro = ref();
async function traigodatos() {
  await useConfiguracion
    .traerConfiguracion()
    .then((resultado) => {
      coloroficialmenu.value = resultado[0].colormenu;
      coloroficialletra.value = resultado[0].colorletra;
      coloroficialtres.value = resultado[0].colortres;
      coloroficialcuatro.value = resultado[0].colorcuatro;
    })
    .catch((error) => {
      console.error("Hubo un error:", error);
    });
}
</script>

<style scoped>
.segundotitulo {
  font-size: 45px;
  color: v-bind(coloroficialletra);
  font-weight: 1000;
  margin-right: 1%;
}

.contebotones {
  justify-content: center;
  align-content: center;
  display: flex;
}

.conteboton {
  font-size: 25px;
  margin-left: 20px;
  margin-right: 20px;
  background-color: v-bind(coloroficialmenu);
  color: v-bind(coloroficialtres);
}

.titulogo {
  padding: 2rem;
  display: flex;
  justify-content: center;
  align-content: center;
  text-align: center;
  margin-top: 70px;
  background-color: rgb(243, 243, 247);
}

.titulo {
  font-size: 45px;
  color: v-bind(coloroficialletra);
  font-weight: 1000;
  margin-right: 1%;
}

.logo {
  height: 70px;
  width: 70px;
}

thead {
  background-color: rgb(145, 145, 145);
}

#archivo {
  width: 90%;
  padding: 10px; /* Agregar un poco de espaciado para que se vea mejor */
  background-color: white; /* Fondo blanco */
  border: 1px solid #ccc; /* Borde gris claro (opcional) */
  border-radius: 5px; /* Bordes redondeados (opcional) */
  color: #333; /* Color de texto (opcional) */
}

.accordion {
  position: relative;
  width: 93%;
  left: 7%;
}

.accordion-button.collapsed:focus {
  background-color: v-bind(coloroficialmenu);
  border-color: rgb(14, 44, 191) !important;
  color: rgb(0, 0, 0) !important;
}

.accordion-button:focus {
  background-color: v-bind(coloroficialmenu);
  border-color: rgb(43, 21, 189) !important;
  color: rgb(0, 0, 0) !important;
}

.accordion-button {
  background-color: v-bind(coloroficialmenu);
  border-color: rgb(255, 255, 255) !important;
  color: rgb(0, 0, 0) !important;
}

@media (max-width: 768px) {
  .contebotones {
    justify-content: center;
    align-content: center;
    display: flex;
    display: grid;
    grid-template-columns: repeat(
      auto-fill,
      minmax(300px, 1fr)
    ); /* Esto hará que las cartas tengan un ancho mínimo de 300px y se expandan automáticamente */
    gap: 10px; /* Espacio entre las cartas */
    justify-content: center;
    align-items: center;
    margin-left: 200px;
    margin-right: 200px;
    margin-bottom: 100px;
  }

  .conteboton {
    width: 100%;
    font-size: 15px;
    margin-left: 20px;
    margin-right: 20px;
  }
}
@media (max-width: 391px) {
  .conteboton {
    font-size: 15px;
  }

  adentroacordion {
    font-size: 14px;
  }

  .contebotones {
    justify-content: center;
    align-content: center;
    display: flex;
    display: grid;
    grid-template-columns: repeat(
      auto-fill,
      minmax(300px, 1fr)
    ); /* Esto hará que las cartas tengan un ancho mínimo de 300px y se expandan automáticamente */
    gap: 10px; /* Espacio entre las cartas */
    justify-content: center;
    align-items: center;
    margin-left: 170px;
    margin-bottom: 200px;
  }

  .titulo {
    font-size: 30px;
    color: #16723b;
    font-weight: 1000;
    margin-right: 1%;
  }

  .segundotitulo {
    font-size: 30px;
    color: #16723b;
    font-weight: 1000;
    margin-right: 1%;
    margin-top: 50px;
  }
}
</style>
