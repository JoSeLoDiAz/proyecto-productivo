<template>
  <div class="contenedor">
    <div class="row">
      <div class="titulogo">
        <div class="titulo">Gestion roles</div>
        <svg
          class="logo"
    ...
    <div class="row">
        ...
        <div class="col-12">
          <div class="buscar">
            <input
              ...
              class="input"
              @input="rol_filtrado"
              placeholder="Buscar rol por codigo"
            />
      ...
      <div class="row" v-if="useroles.cargando === false">
        <div class="col-12">
          <div class="table-container" style="text-transform: capitalize">
            <table class="table table-striped">
              ...
              <tbody v-for="r in roles" :key="r">
                <tr>
                  <td>{{ r.denominacion }}</td>
                  <td style="cursor: pointer" @click="cambiarEstado(r)">
                    <strong v-if="r.estado === true" style="color: green"
                      >Activo</strong
                    >
      ...
      >
        <div class="modal-dialog">
          <div class="modal-content">
            ...
            <div style="top: -15px; position: relative" v-if="alert === true">
...
async function traerRoles() {
  await useroles.buscarRol().then((res) => {
    roles.value = res;
    newRoles.value = [...roles.value]
  });
}

function rol_filtrado(event){
  buscado_v.value=event.target.value;
  if (buscado_v.value === "") {
    vacio.value = "";
    traerRoles();
  } else {
    roles.value = newRoles.value;
    let filtrado = roles.value.filter((g) =>
...
async function registrarRol() {
  ...
  await useroles
    .registrarRol({
      denominacion: denominacion.value.toLowerCase(),
      estado: true,
    })
    .then(() => {
      traerRoles();
...
async function editaRol() {
  if(validaciones()===true){
  await useroles
    .editarRol(id.value, denominacion.value.toLowerCase())
    .then(() => {
      traerRoles();
      ok();
      closeModal();
...
function cambio(msg) {
  if (msg == "Agregar") {
    msg_modal.value = "Agregar Rol";
    limpiar();
  } else {
    msg_modal.value = "Editar Rol";
  }
}
...
async function cambiarEstado(r) {
  ...
  await useroles.cambiarEstado(r._id, r.estado).then(() => {
    console.log("Cambio de estado realizado correctamente");
  });
}

function limpiar() {
  (codigo.value = ""), (denominacion.value = "");
}
...
onMounted(async () => {
  setDocumentTitle(), await traerRoles(), await traigodatos();
});
