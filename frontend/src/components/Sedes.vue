<template>
  <div id="app">
    ...
    <div v-if="useSede.cargando == false">
      <div class="row">
        <div class="col-12">
          <div class="table-container">
            <table class="table table-striped">
              ...
              <tbody v-for="s in sedeEncontrada" :key="s">
                <tr>
                  <td>
                    {{ s.nombre }}
                  </td>
                  ...
                  <td style="cursor: pointer" @click="cambiarEstado(s)">
                    <strong v-if="s.estado === true" style="color: green"
                      >Activo</strong
                    >
...
let errores = ref("");
let alert = ref(false);
let nombreCentro = ref("");
let newSedes = ref([]);

function cambio(msg) {
  if (msg == "Agregar") {
    msg_modal.value = "Agregar Sede";
    limpiar();
  } else {
    msg_modal.value = "Editar Sede";
  }
}

async function buscarSede() {
  console.log(useCentro.idCentro);
  await useCentro.buscarCentroId(useCentro.idCentro._id);
  nombreCentro.value = useCentro.idCentro.nombre;
  await useSede.buscarSede().then((res) => {
    let filtrado = res.filter((a) => a.centro === useCentro.idCentro._id);
    sedeEncontrada.value = filtrado;
    newSedes.value = [...sedeEncontrada.value];
  });
}

function sede_filtrado(event) {
  buscado_v.value = event.target.value;
  if (buscado_v.value === "") {
    vacio.value = "";
    buscarSede();
  } else {
    sedeEncontrada.value = newSedes.value;
    let filtrado = sedeEncontrada.value.filter((g) =>
      g.nombre.includes(buscado_v.value.toLowerCase())
    );
    sedeEncontrada.value = [];
...
function validaciones() {
  if (nombre.value.trim() === "") {
    ...
    alerta();
  } else if (contacto.value.trim() === "") {
    alert.value = true;
    errores.value = "Complete el campo Contacto";
    alerta();
  } else {
    return true;
  }
...
async function registrarSede() {
  ...
  await useSede
    .registrarSede({
      ...
      contacto: contacto.value.toLowerCase(),
      centro: useCentro.idCentro._id,
      estado: true,
    })
    .then(() => {
      buscarSede();
...
async function cambiarEstado(s) {
  ...
  await useSede.cambiarEstado(s._id, s.estado).then(() => {
    console.log("Cambio de estado realizado correctamente");
  });
}

function limpiar() {
  (nombre.value = ""), (direccion.value = ""), (contacto.value = "");
  centro.value = "";
}
