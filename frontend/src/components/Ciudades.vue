  ...
  <div>
    ...
    <div class="buscar">
      <div class="row m-0 p-0 d-flex align-items-center">
        <div class="col-1 col-md-1 d-md-block d-none"></div>
        <div class="col-12 col-md-10 d-flex justify-content-between">
          <div class="inputimg">
            <input
              v-model="buscado_v"
              @input="ciudad_filtrado"
              type="text"
    ...
    <div v-if="useCiudad.cargando == false">
      <div class="table-container">
        <table class="table table-striped">
          ...
          <tbody>
            <tr v-for="c in ciudadEncontrada" :key="c">
              <td>{{ c.nombre }}</td>
              <td>{{ c.daneciudad }}</td>
              <td>{{ c.region }}</td>
              <td>{{ c.departamento }}</td>
              <td>{{ c.danedepartamento }}</td>
              <td
                style="cursor: pointer; font-weight: 600"
                @click="cambiarEstado(c)"
              >
                <span v-if="c.estado === true" style="color: green"
                  >Activo</span
                >
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <div class="d-flex flex-row mb-2 flex-wrap">
              ...
              <div class="p-2">
                <span>DANE Ciudad</span>
                <input type="text" v-model="daneciudad" />
              </div>
              ...
              <div class="p-2">
                ...
                <input type="text" v-model="departamento" />
              </div>

              <div class="p-2">
                <span>DANE Departamento</span>
                <input type="text" v-model="danedepartamento" />
              </div>
            </div>
          </div>
          ...
          <div class="modal-footer" style="margin: auto">
            ...
            <button
              ...
              v-if="msg_modal == 'Agregar Ciudad'"
              type="button"
              class="btn"
              @click="registrarCiudad()"
            >
              Agregar
            </button>
            <button
...
function cambio(msg) {
  if (msg == "Agregar") {
    msg_modal.value = "Agregar Ciudad";
    limpiar();
  } else {
    msg_modal.value = "Editar Ciudad";
  }
}

async function buscarCiudad() {
  await useCiudad.buscarCiudad().then((res) => {
    ciudadEncontrada.value = res;
  });
}

const ciudad_filtrado = computed(() => {
  if (buscado_v.value === "") {
    buscarCiudad();
    vacio.value = "";
  } else {
    let filtrado = ciudadEncontrada.value.filter((g) =>
      g.nombre.includes(buscado_v.value.toLowerCase())
    );
    if (filtrado.length === 0) {
      vacio.value = `Sin resultados para ${buscado_v.value}`;
      ciudadEncontrada.value = [];
    } else {
      vacio.value = "";
      ciudadEncontrada.value = filtrado;
    }
  }
});

async function registrarCiudad() {
  await useCiudad
    .registrarCiudad({
      nombre: nombre.value.toLowerCase(),
      daneciudad: daneciudad.value.toLowerCase(),
      region: region.value.toLowerCase(),
      departamento: departamento.value.toLowerCase(),
      danedepartamento: danedepartamento.value.toLowerCase(),
      estado: true,
    })
    .then(() => {
      buscarCiudad();
...
async function editar(c) {
  ...
  (nombre.value = c.nombre),
    (daneciudad.value = c.daneciudad),
    (region.value = c.region),
    (departamento.value = c.departamento),
    (danedepartamento.value = c.danedepartamento);
}
...
async function guardarEdision() {
  await useCiudad
    .editarCiudad(
      ...
      daneciudad.value.toLowerCase(),
      region.value.toLowerCase(),
      departamento.value.toLowerCase(),
      danedepartamento.value.toLowerCase()
    )
    .then(() => {
      buscarCiudad();
      ok();
...
async function cambiarEstado(c) {
  ...
  await useCiudad.cambiarEstado(c._id, c.estado).then(() => {
