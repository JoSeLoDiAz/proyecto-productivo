<template>
  <div>
    <div class="titulogo">
      <router-link style="padding-left: 10px;margin: 0 50px;" to="infoprograma">
          <img src="../assets/volver.png" class="volver" alt="">
        </router-link>
      ...
      <svg class="logo" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:cc="http://creativecommons.org/ns#"
        ...
        xmlns:inkscape="http://www.inkscape.org/namespaces/inkscape" width="100" height="98" viewBox="0 0 74.999997 73.5"
        version="1.1" id="svg12" sodipodi:docname="SENA logo.svg" inkscape:version="0.92.4 (5da689c313, 2019-01-14)">
        ...
        <sodipodi:namedview pagecolor="#ffffff" bordercolor="#666666" borderopacity="1" objecttolerance="1"
          gridtolerance="1" guidetolerance="1" inkscape:pageopacity="0" inkscape:pageshadow="2"
          inkscape:window-width="1366" inkscape:window-height="705" id="namedview14" showgrid="false" units="px"
          inkscape:showpageshadow="false" inkscape:zoom="1" inkscape:cx="96.265104" inkscape:cy="56.747294"
          inkscape:window-x="-8" inkscape:window-y="-8" inkscape:window-maximized="1" inkscape:current-layer="svg12">
          <inkscape:grid type="xygrid" id="grid58" />
        </sodipodi:namedview>
        <g id="#ffffffff" transform="translate(0,-130.5)" />
        <path :style="{backgroundColor: coloroficialcuatro}"
    ...
    <div class="row">
      <div class="row">
        <div class="col-12 buscar">
          <input class="input" type="text" @input="proyecto_filtrado"
            placeholder="Buscar por codigo" />
            <img class="buscadorr" src="../assets/buscar.png" alt="" />

          <button v-if="useUsuario.rol === 'Gestor'.toLowerCase()" style="position: relative; left: 28%" type="button"
            class="btn  botonagregar" data-bs-toggle="modal" data-bs-target="#staticBackdrop"
      ...
      <div v-if="useProyecto.cargando === false">
        <div>{{ vacio }}</div>

        <div class="row" v-for="(p,i) in proyectosEncontrados" :key="p">
          <div class="accordion accordion-flush" id="accordionFlushExample">
            <div class="accordion-item">
              <h2>
                <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                  :data-bs-target="'#collapse-' + p._id" aria-expanded="false" :aria-controls="'collapse-' + p._id"
                  style="font-weight: 900; text-transform: capitalize;">
                  Nombre: {{ p.nombre }}
                </button>
              </h2>
              <div :id="'collapse-' + p._id" :class="i===0?'accordion-collapse collapse show':'accordion-collapse collapse'" data-bs-parent="#accordionFlushExample">
                <div class="accordion-body">
                  <p>
                    <span style="font-weight: 1000">Descripcion:</span>
                    {{ p.descripcion }}
                  </p>
                  <p>
                    <span style="font-weight: 1000">Fecha:</span>
                    {{ p.fecha }}
                  </p>
                  <p>
                    <span style="font-weight: 1000">Version:</span>
                    {{ p.version }}
                  </p>
                  <p>
                    <span style="font-weight: 1000">Programa:</span>
                    {{ p.programa.nombre }}
                  </p>

                  <p>
                    <span style="font-weight: 1000">Estado:</span>
                  ...
                  <ul class="wrapper">
                    ...
                    <li v-if="useUsuario.rol === 'Gestor'.toLowerCase()" @click="cambiarEstado(p)" class="icon black">
                      <span class="tooltip">Cambiar estado</span>
                      <img style="width: 30px; margin: 10px; cursor: pointer" src="../assets/cambio_estado.png" alt="">
                    </li>



                    <li v-if="useUsuario.rol === 'Gestor'.toLowerCase()" data-bs-toggle="modal" data-bs-target="#staticBackdrop"
                      @click="cambio('Editar'), editar(p)" class="icon black">
                      <span class="tooltip">Editar</span>
                      <img style="width: 30px; margin: 10px; cursor: pointer" src="../assets/editar.png" alt="">
                    </li>


                    <li class="icon black">
                      <span class="tooltip">Descargar</span>
                      <a :href="p.archivo" target="_blank">
                        <img style="width: 30px; margin: 10px; cursor: pointer" src="../assets/descargar.png" alt="" />
                      </a>
                    </li>
        ...
        <div class="modal-content">
          ...
          <div class="modal-body d-flex flex-column">
            ...
            <div class="input-group input-group-sm mb-3">
              <input v-model="nombre" type="text" class="form-control" aria-label="Sizing example input"
                aria-describedby="inputGroup-sizing-sm" />
            </div>

            <span>Descripcion</span>
            <div class="input-group input-group-sm mb-3">
              <input v-model="descripcion" type="text" class="form-control" aria-label="Sizing example input"
                aria-describedby="inputGroup-sizing-sm" />
            </div>

            <span>Fecha</span>
            <div class="input-group input-group-sm mb-3">
              <input v-model="fecha" type="date" class="form-control" aria-label="Sizing example input"
                aria-describedby="inputGroup-sizing-sm" />
            </div>

            <span>Version</span>
            <div class="input-group input-group-sm mb-3">
              <input v-model="version" type="text" class="form-control" aria-label="Sizing example input"
                aria-describedby="inputGroup-sizing-sm" />
            </div>

            <span>Archivo</span>
            <span class="mi-archivo">
              <input type="file" accept=".pdf, .doc, .docx" @change="llevar" id="mi-archivo" name="mi-archivo" />
            </span>
            <label for="mi-archivo">
              ...
              <span style="margin: 0 10px" v-if="archivo === null">Archivo</span>
          ...
          <div v-if="alert === true">
            ...
            <div class="alert alert-warning d-flex align-items-center" role="alert">
              <svg style="width: 20px; height: 20px" class="bi flex-shrink-0 me-2" role="img" aria-label="Warning:">
          ...
          <div v-if="useProyecto.cargando === true" class="row d-flex justify-content-center">
            <div class="spinner-border text-success" role="status">
              <span class="visually-hidden">Loading...</span>
            </div>
          </div>
          ...
          <div class="modal-footer" style="margin: auto">
            <button :style="{color: coloroficialtres}" type="button" class="btn btn-danger" data-bs-dismiss="modal" aria-label="Close" id="close">
              Cancelar
            </button>
            <button :style="{color: coloroficialtres, backgroundColor:coloroficialmenu}" v-if="msg_modal == 'Agregar Proyecto'" type="button" class="btn btn-success"
              @click="registrarProyecto()">
              Agregar
            </button>
            <button :style="{color: coloroficialtres, backgroundColor:coloroficialmenu}"  v-else type="button" class="btn btn-success" @click="guardarEdision()">
              Editar
            </button>
          </div>
        </div>
...
import { useProyectoStore } from "../stores/proyecto.js";
...
let errores = ref("");
let alert = ref(false);
let newProyectos = ref([]);

function cambio(msg) {
  if (msg == "Agregar") {
    limpiar();
    msg_modal.value = "Agregar Proyecto";
  } else {
    msg_modal.value = "Editar Proyecto";
  }
}

async function buscarProyectos() {
  await useProyecto.buscarProyecto().then((res) => {
    const usuarioCumpleCondicion = res.filter(
      (p) => p.programa._id === usePrograma.idPrograma._id
    );
    if (usuarioCumpleCondicion.length > 0) {
      proyectosEncontrados.value = usuarioCumpleCondicion;
      newProyectos.value = [...proyectosEncontrados.value];
    }
  });
}
...
function proyecto_filtrado(event){
  buscado_v.value=event.target.value;
  if (buscado_v.value === "") {
    buscarProyectos();
    vacio.value = "";
  } else {
    proyectosEncontrados.value = newProyectos.value;
    let filtrado = proyectosEncontrados.value.filter(
      (g) => g.nombre.includes(buscado_v.value.toLowerCase())
    );
    if (filtrado.length === 0) {
      vacio.value = `Sin resultados para ${buscado_v.value}`;
      proyectosEncontrados.value = [];
    } else {
      vacio.value = '';
      proyectosEncontrados.value = filtrado;
    }
  }
}
...
function validaciones(){
  if(nombre.value.trim()===''){
    ...
    alert.value = true;
        errores.value = 'complete el campo Descripción';
        alerta();
  }else if(fecha.value.trim()===''){
    alert.value = true;
        errores.value = 'complete el campo Fecha';
        alerta();
  }else if(version.value.trim()===''){
    alert.value = true;
        errores.value = 'complete el campo Versión';
        alerta();
  }else if(archivo.value.trim()===''){
    alert.value = true;
...
async function registrarProyecto() {
  ...
  await useProyecto
    .registrarProyecto({
      ...
      programa: usePrograma.idPrograma._id,
      estado: true,
    })
    .then(() => {
      ok();
...
function editar(p) {
  (id.value = p._id),
    ...
    (archivo.value = p.archivo),
    (programaId.value = p.programa._id);
}
...
async function guardarEdision() {
  ...
  await useProyecto
    .editarProyecto(
      ...
      descripcion.value.toLowerCase(),
      fecha.value.toLowerCase(),
...
async function cambiarEstado(p) {
  ...
  await useProyecto.cambiarEstado(p._id, p.estado).then(() => {
...
onMounted(async()=>{
  await buscarProyectos(),traigodatos()
});
...
.accordion-button.collapsed:focus {
  background-color: v-bind(coloroficialmenu);
  border-color: rgb(14, 44, 191) !important;
  color: rgb(0, 0, 0) !important;
}

.accordion-button:focus {
  background-color: v-bind(coloroficialmenu);
  border-color: rgb(43, 21, 189) !important;
  color: rgb(0, 0, 0) !important;
}
